...
# 방문객 수와 전기차 대수 데이터만 추출
df = cluster_df[['방문객수', 'electornic_car']].values
...
# KMeans 시각화
plt.subplot(1, 2, 1)
plt.scatter(df_scaled[:, 0], df_scaled[:, 1], c=kmeans_labels, cmap='viridis', marker='o')
plt.title('KMeans Clustering Results (Standardized)')
plt.xlabel('Standardized Visitor Count')
plt.ylabel('Standardized EV Count')

# DBSCAN 시각화
plt.subplot(1, 2, 2)
plt.scatter(df_scaled[:, 0], df_scaled[:, 1], c=dbscan_labels, cmap='viridis', marker='o')
plt.title('DBSCAN Clustering Results (Standardized)')
plt.xlabel('Standardized Visitor Count')
plt.ylabel('Standardized EV Count')
